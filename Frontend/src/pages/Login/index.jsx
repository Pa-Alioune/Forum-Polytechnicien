  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!err?.response) {
        ...
        if (err.response.data.password) setErreur(err.response.data.password);
        else if (err.response.data?.email) setErreur(err.response.data.email);
  ...
  return (
    <Container imgUrl={backgroundImage}>
      <Header>
        ...
        <BoiteAlerte
          erreur={erreur}
          text="Vérifier vos informations fournies"
        />
        <div>
      ...
      <StyledFormWrapper>
        ...
        <FormStyled onSubmit={handleSubmit}>
          <InputGroupStyled>
            <div>
              <LabelStyled htmlFor="email">Votre adresse email</LabelStyled>
            </div>
            <div>
              <InputStyled
          ...
          <InputGroupStyled>
            <div>
              <LabelStyled htmlFor="password1">Votre mot de passe</LabelStyled>
            </div>
            <div>
              <InputStyled
          ...
          <LoginSetGroup>
            <CheckboxGroupStyled>
              <input
                onChange={onSouvenirChange}
                type="checkbox"
                id="souvenir"
              />
              <LabelLoginSet htmlFor="souvenir">
                Se souvenir de moi
              </LabelLoginSet>
            </CheckboxGroupStyled>
            <div>
          ...
          <div>
            <ButtonStyled type="submit">Connexion</ButtonStyled>
          </div>
        </FormStyled>
        <ButtonStyled2 to="/">
          <LogoGoogle src={googleLogo} alt="" />{" "}
          <div> Continuez avec Google</div>
        </ButtonStyled2>
        <div>
          <Styledparag2 to="/register">
            Je ne posséde pas de compte
          </Styledparag2>
        </div>
      </StyledFormWrapper>
    </Container>
